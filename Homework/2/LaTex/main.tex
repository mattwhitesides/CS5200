...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \item \textbf{4.5-1} Use the master method to give tight asymptotic bounds for the following recurrences. For all we have $a = 2, b = 4$ so for example we have three cases:
    ...
    \begin{enumerate}
      ...
      Here we have $f(n) = 1$ when $\epsilon = 1$ so we go with case 1, therefor \bm{$T(n) = \Theta(n^{\log_4 2})$}.
      \item \[T(n) = 2T(n \mathbin{/} 4) + \sqrt{n}\]
      Here we have $f(n) = \sqrt{n}$ which equals case 2 so \bm{$T(n) = \Theta(n^{\log_4 2} lg(n))$}.
