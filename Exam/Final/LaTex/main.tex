...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
        ...
        If $N = 4$ we have a graph like $V = \{A, B, C, John\}$ then if F is a function of handsakes 
...
B-Tree Times Standard Dev: 0.1647701717343564      
    ...
    \begin{enumerate}
        ...
        and since d is the gcd of (p, q) d/2 must be the maximum that the gcd of (p/2, q/2) could be. and obvisouly (d/2) = (d * 2). \\

        Also you could just say if:
        ...
        If $p$ is odd and $q$ is even then obvisouly $p \neq q$,
        ...
        The main algorithm is in the gcd\_util() method as the main GCD converts the given number to a binary string and keeps itself seperate from the recursion.
